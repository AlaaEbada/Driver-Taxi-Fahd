...
const MilanSwitzerlandContent = () => {
    ...
    return (
        <div className="bg-background min-h-screen">
            ...
            <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0">
                    <Image
                        ...
                        alt={language === 'ar' ? 'ميلانو وسويسرا' : 'Milan and Switzerland'}
                        fill
                ...
                <div className="relative z-10 text-center container-luxury px-4">
                    ...
                    >
                        <span className="inline-block px-4 py-2 rounded-full bg-gold/80 text-primary border border-white/30 mb-6 backdrop-blur-sm font-bold shadow-lg">
                            {language === 'ar' ? 'مع أبو فهد وتاكسي فهد' : 'With Abu Fahd & Taxi Fahd'}
                        </span>
                        ...
                        <p className="text-xl text-white font-medium max-w-3xl mx-auto leading-relaxed drop-shadow-sm">
                            {language === 'ar'
                                ? 'نبذة سياحية تجمع بين سحر التاريخ الإيطالي وجمال الطبيعة السويسرية'
            ...
            <div className="container-luxury py-16 space-y-24">
                ...
                <div className="space-y-20">
                    ...
                    <section className="grid lg:grid-cols-2 gap-12 items-center">
                                    ...
                                    {[
                                        {
                                            ...
                                            desc: language === 'ar' ? 'عاصمة الموضة والتسوق، ونقطة الانطلاق المثالية للرحلات نحو سويسرا وبحيرات الشمال.' : 'Fashion and shopping capital, the perfect gateway for trips to Switzerland and northern lakes.'
                                        },
                                        {
                                            title: language === 'ar' ? 'روما (الوسط):' : 'Rome (Central):',
                                            desc: language === 'ar' ? 'التاريخ العظيم، حيث الكولوسيوم ونافورة تريفي. مدينة لا تكتمل الرحلة بدونها.' : 'Great history, with the Colosseum and Trevi Fountain. A city no trip is complete without.'
                                        },
                                        {
                        ...
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className={`${isRTL ? 'order-1 lg:order-2' : 'order-1'} relative h-[400px] rounded-3xl overflow-hidden shadow-2xl border-4 border-white transform rotate-0 hover:rotate-2 transition-transform duration-500`}
                        >
                            <Image
                    ...
                    <section className="grid lg:grid-cols-2 gap-12 items-center">
                        <motion.div
                            ...
                            whileInView={{ opacity: 1, scale: 1 }}
                            viewport={{ once: true }}
                            className="order-1 relative h-[400px] rounded-3xl overflow-hidden shadow-2xl border-4 border-white transform rotate-0 hover:-rotate-2 transition-transform duration-500"
                        >
                            <Image
                        ...
                        >
                            ...
                            <div className={`prose prose-lg text-muted-foreground leading-relaxed ${isRTL ? 'text-right' : 'text-left'}`}>
                                ...
                                <ul className="space-y-4 mt-4 list-none p-0">
                                    {[
                                        { title: language === 'ar' ? 'انترلاكن:' : 'Interlaken:', desc: language === 'ar' ? 'القلب النابض للسياحة، تقع بين بحيرتين وتحيط بها قمم جبال الألب. المميزة للأنشطة الجبلية.' : 'The heart of tourism, nestled between two lakes and surrounded by Alpine peaks. Famous for mountain activities.' },
                                        ...
                                        { title: language === 'ar' ? 'لوجانو:' : 'Lugano:', desc: language === 'ar' ? 'مزيج فريد. الطبيعة والجبال سويسرية، لكن الروح واللغة والعمارة إيطالية.' : 'A unique blend. Nature and mountains are Swiss, but the spirit, language, and architecture are Italian.' },
                                        { title: language === 'ar' ? 'زيرمات:' : 'Zermatt:', desc: language === 'ar' ? 'قرية جبلية خالية من السيارات، تشتهر بقمة "ماترهورن" الشهيرة (غلاف شوكولاتة توبليرون).' : 'A car-free mountain village, famous for the "Matterhorn" peak (Toblerone chocolate wrapper).' }
                                    ].map((item, idx) => (
                                        <li key={idx} className="flex gap-3">
                                            <CheckCircle2 className="w-6 h-6 text-gold shrink-0 mt-1" />
                                            <span><strong>{item.title}</strong> {item.desc}</span>
                                ...
                                <p className={`mt-4 bg-primary/5 p-4 rounded-xl border border-primary/10 ${isRTL ? 'border-r-4' : 'border-l-4'}`}>
                                    ...
                                    {language === 'ar'
                                        ...
                                        : ' Complete peace, extreme punctuality, quality chocolate and cheeses, and trains considered the most beautiful in the world.'}
                                </p>
                            </div>
                        </motion.div>
                    </section>
                ...
                <section className="bg-muted/30 p-8 md:p-12 rounded-3xl border border-border text-center">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        ...
                        <div className={`bg-background p-6 rounded-xl shadow-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                            ...
                            <p className="text-sm">
                                {language === 'ar' ? 'إيطاليا تستخدم اليورو، سويسرا تستخدم الفرنك السويسري (لكن أغلب الأماكن تقبل اليورو وترجع الباقي بالفرنك).' : 'Italy uses Euro, Switzerland uses Swiss Franc (though most places accept Euro and give change in Francs).'}
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <section>
                    <div className="text-center mb-12">
                        ...
                        <p className="text-muted-foreground">{language === 'ar' ? 'إيطاليا & سويسرا مع أبو فهد' : 'Italy & Switzerland with Abu Fahd'}</p>
                    </div>
                    ...
                    <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-gold/50 before:to-transparent">
                        ...
                        <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                            ...
                            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-card p-6 rounded-2xl border border-gold/20 shadow-lg hover:shadow-xl transition-all">
                                <h3 className={`text-xl font-bold text-primary mb-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                                    {language === 'ar' ? 'المحطة الأولى: ميلانو وبحيراتها (3 أيام)' : 'Station 1: Milan & Its Lakes (3 Days)'}
                                </h3>
                                <ul className={`space-y-3 text-muted-foreground text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                                    <li><strong>{language === 'ar' ? 'اليوم 1:' : 'Day 1:'}</strong> {language === 'ar' ? 'الوصول إلى ميلانو، استكشاف "الدومو" والتسوق في "جاليريا فيتوريو إيمانويل". (لا تحتاج سائق داخل المدينة).' : 'Arrival in Milan, Duomo exploration, and Galleria Vittorio Emanuele shopping. (No driver needed in city).'}</li>
                                    <li><strong>{language === 'ar' ? 'اليوم 2:' : 'Day 2:'}</strong> {language === 'ar' ? 'بحيرة كومو (رحلة نهارية). زيارة "بيلاجيو" و"فارينا". يفضل سائق خاص للاستمتاع بالمناظر دون قلق المواقف.' : 'Lake Como (Day Trip). Visit Bellagio and Varenna. Private driver recommended to enjoy views without parking stress.'}</li>
                                    <li><strong>{language === 'ar' ? 'اليوم 3:' : 'Day 3:'}</strong> {language === 'ar' ? 'لوجانو السويسرية. مدينة سويسرية بروح إيطالية. زيارة "سويس مينيوتور" وقمة "سان سالفاتوري". يمكن العودة لميلانو أو المبيت هناك.' : 'Lugano, Switzerland. Swiss city with an Italian spirit. Visit Swissminiatur and San Salvatore peak. Return to Milan or stay overnight.'}</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                            ...
                            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-card p-6 rounded-2xl border border-gold/20 shadow-lg hover:shadow-xl transition-all">
                                ...
                                <ul className={`space-y-3 text-muted-foreground text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                                    <li><strong>{language === 'ar' ? 'اليوم 4:' : 'Day 4:'}</strong> {language === 'ar' ? 'الانتقال إلى انترلاكن (3 ساعات من ميلانو). أحد أجمل طرق القيادة في العالم عبر "نفق غوتارد".' : 'Transfer to Interlaken (3 hours from Milan). One of the world\'s most beautiful drives via Gotthard Tunnel.'}</li>
                                    <li><strong>{language === 'ar' ? 'اليوم 5:' : 'Day 5:'}</strong> {language === 'ar' ? 'قمة "يونغفراو يوخ" (قمة أوروبا). ركوب القطار المسنن واللعب بالثلوج حتى في الصيف.' : 'Jungfraujoch (Top of Europe). Cogwheel train ride and snow play even in summer.'}</li>
                                    <li><strong>{language === 'ar' ? 'اليوم 6:' : 'Day 6:'}</strong> {language === 'ar' ? 'وادي "لوتربرونين" وقرية "جريندلوالد". وادي الـ 72 شلالاً، مكان خيالي للتصوير والمشي.' : 'Lauterbrunnen Valley and Grindelwald. Valley of 72 waterfalls, a dream spot for photos and hiking.'}</li>
                                    <li><strong>{language === 'ar' ? 'اليوم 7:' : 'Day 7:'}</strong> {language === 'ar' ? 'بحيرة "براينز" وشلالات "جيس باخ". استجمام في البحيرة الفيروزية وزيارة الجراند أوتيل التاريخي.' : 'Lake Brienz and Giessbach Falls. Relaxation by the turquoise lake and visiting the historic Grand Hotel.'}</li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                            ...
                            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-card p-6 rounded-2xl border border-gold/20 shadow-lg hover:shadow-xl transition-all">
                                ...
                                <ul className={`space-y-3 text-muted-foreground text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                                    <li><strong>{language === 'ar' ? 'اليوم 8:' : 'Day 8:'}</strong> {language === 'ar' ? 'التوجه إلى فيرونا وبحيرة جاردا. زيارة قرية "سيرميوني" التاريخية وسط البحيرة.' : 'Heading to Verona and Lake Garda. Visit historic Sirmione village in the lake center.'}</li>
                                    <li><strong>{language === 'ar' ? 'اليوم 9:' : 'Day 9:'}</strong> {language === 'ar' ? 'فيرونا (مدينة الحب). زيارة شرفة جولييت وساحة "أرينا" الرومانية.' : 'Verona (City of Love). Visit Juliet\'s Balcony and the Roman Arena.'}</li>
                ...
                <section className="bg-primary text-primary-foreground p-8 md:p-12 rounded-3xl relative overflow-hidden">
                    ...
                    <div className="mt-12 text-center">
                        <Button
                            asChild
                            size="lg"
                            className="bg-gold text-primary hover:bg-gold/90 font-bold text-lg md:text-xl px-12 py-8 rounded-full shadow-lg hover:scale-105 transition-transform"
                        >
...
export default MilanSwitzerlandContent;
